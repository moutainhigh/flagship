...
<html>
			...
			editForm.form('submit',{
				...
				onSubmit:function(){
					if (editForm.form("validate")) {
						...
						if(status == 0){
							$.messager.alert('提示信息', '该员工已离职,不能修改', 'info');
							return false;
						}
  ...
  <body style="background: white;">
  	<form id="editForm" class="easyui-form" method="post" modelAttribute="user">
  		<input type="hidden" id="id" name="id" value="${user.id}"/>
  		<input type="hidden" name="oldusername" value="${user.username}"/>
  		<input type="hidden" name="oldmobile" value="${user.mobile}"/>
  		<!-- 是否离职 -->
		...
		<table class="tableForm" border="1" width="100%" >
			<tr>
				...
				<td width="35%">
					<input type="text" id="name" name="name"  value="${user.name}" class="easyui-textbox" data-options="required:true,validType:['length[0,10]']" style="width: 175px;height: 24px;"/>
				</td>
				<td width="15%" class="tdR"><span style="color: red">*</span>用户名:</td>
				<td width="35%">
					<input type="text" id="username" name="username"  value="${user.username}" class="easyui-textbox" data-options="required:true,validType:['length[0,20]']" style="width: 175px;height: 24px;"/>
				</td> 
			</tr>
			<tr>
				<td class="tdR"><span style="color: red">*</span>手机号:</td>
				<td>
					<input type="text" id="mobile" name="mobile"  value="${user.mobile}" class="easyui-textbox" data-options="required:true,validType:['length[0,11]','mobileIsValid']" style="width: 175px;height: 24px;"/>
				</td>
				
				<td class="tdR"><span style="color: red">*</span>设置权限:</td>
				<td>
					<!-- 存储修改之前的权限，后台判断权限是否改变 -->
					<input type="hidden" value = "${user.auid}" name="oldauth" >
				    <!-- 修改页面 选择用于回显 -->
					<input type="hidden" value = "${user.auid}"name="oauth"  id="permissionIds">
			...
			<tr>
				...
				<td><input type="hidden" name="oldSort" value="${user.sortId}" />
					<input style="width: 180px;" class="easyui-combobox" name="sortId"
					id="sort" value="${user.sortId}"
					data-options="
				...
				<td class="tdR">员工编号：</td>
  					<td>
  						<input type="text" data-options="events:{focus:openSearch}"  id="employeeNo" name="employeeNo" value="${user.employeeNo}" class="easyui-textbox"  style="width: 175px;height: 24px;" readonly="true" />
  					</td>
			</tr>
			<tr>
				<td class="tdR">私信设置:</td>
					<td colspan="3">
					<input type="hidden" name="oldLetter" value="${user.letterId}"/>
						<input style="width: 400px;"   class="easyui-combobox" 
							name="letter" id="letter" value="${user.letterId}"
						data-options="
								url:'${app }/app/getAllUser?id='+${user.id},
								method:'get',
								valueField:'id',
								textField:'name',
								multiple: true ,
